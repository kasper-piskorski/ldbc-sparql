...
def generate_seed_dict_bi(row, query_num, DATE_FORMAT):
    if query_num == 1:
        ...
        return {"personId": row[0], "countryName": row[1], "tagClassName": row[2], "minPathDistance": row[3],
                "maxPathDistance": row[4]}
    elif query_num == 17:
        return {"countryName": row[0]}
    elif query_num == 18:
...
def generate_seed_dict_ic(row, query_num, DATE_FORMAT):
    if query_num == 1:
        ...
        return {"personId": row[0], "startDate": start_date, "durationDays": row[2], "countryXName": row[3],
                "countryYName": row[4]}
    elif query_num == 4:
        start_date = dt.fromtimestamp(int(row[1]) / 1000).strftime(DATE_FORMAT)
